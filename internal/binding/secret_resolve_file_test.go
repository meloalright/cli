...
func TestResolveFileRef_RelativePath_StillRejected(t *testing.T) {
	// Regression guard: cwd-relative input is still rejected even though tilde
	// was loosened. Guards against a future widening of expandTildePath that
	// would also expand "./..." (and thereby weaken the absolute-path audit).
	ref := &SecretRef{Source: "file", ID: SingleValueFileRefID}
	pc := &ProviderConfig{
		Source:            "file",
