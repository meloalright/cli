...
- 用户说"给表格添加一行""删除第二列""合并单元格""把 A1:C3 合并""拆分""设置表头""在表头加背景色""调整列宽""单元格背景色""垂直居中" → 使用 `table_*` 指令（见下方「文档内嵌表格操作」）
...
| 指令 | 用途 | 关键参数 |
|------|------|----------|
| `table_insert_rows` | 插入行 | `--row-index`（1-based；`0`=首行之前，`-1`=末尾追加） |
| `table_insert_cols` | 插入列 | `--col`（字母；`0`=首列前，`-1`=末尾） |
| `table_delete_rows` | 删除行范围 | `--row-start`, `--row-end`（**1-based** 左闭右开） |
| `table_delete_cols` | 删除列范围 | `--col-start`, `--col-end`（字母，两端都包含） |
| `table_merge_cells` | 合并单元格 | `--range`（A1:C3，两端都包含） |
| `table_unmerge_cells` | 拆分单元格 | `--cell`（A1 记法） |
| `table_update_property` | 不带 `--cell` = 表级（`--col-width` / `--header-row` / `--header-column`）；带 `--cell` = 单元格（`--background-color` / `--vertical-align`） | 见参考文档 |
